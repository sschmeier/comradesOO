...
comradesDataSet <- function(rnas,
    ...
    hybFiles[[ "original"]] = swapHybs(hybList = hybFiles[[ "all" ]][[ "all" ]],
                                       rna = rnas )
    
    hybFiles[[ "host"]] = swapHybs3(hybList = hybFiles[[ "all" ]][[ "all" ]],
                                    rna = rnas )
    print(" *** RNA of interest Alone ***")
    hybFiles[[ "noHost"]] = swapHybs2(hybList = hybFiles[[ "all" ]][[ "all" ]],
                                      rna = rnas )
    ...
    print(i)
    matrixList[[ "noHost" ]] = list()
    matrixList[[ "noHost" ]] = getMatrices(hybFiles[[ "noHost"]],
                                           rnas, rnaSize)
    names(matrixList[[ "noHost" ]]) = sampleNames
    
    matrixList[[ "original" ]] = list()
    matrixList[[ "original" ]] = getMatrices(hybFiles[[ "original"]],
                                             rnas, rnaSize)
    names(matrixList[[ "original" ]]) = sampleNames
    c = c +1
